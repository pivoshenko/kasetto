...
function Main {
    ...
    $artifact = "kasetto-windows-${arch}.exe"

    Log "installing kasetto $version (windows/$arch)"
    ...
    try {
        ...
        $binaryPath = Join-Path $tmpDir "kasetto.exe"
        ...
        Test-Checksum $binaryPath $checksumsPath $artifact
        ...
        Copy-Item $binaryPath $kstDest -Force
