...
`rules` land in each agent's native instruction file or directory rather than the skills path
above. Paths are verified against each agent's official docs. **Aggregate** files (one shared file,
many rules merged via managed comment blocks so your own edits survive) and **directories** (one
file per rule) are noted below; project paths are relative to the repo root, global paths to `$HOME`.

| Agent          | Project rules path                   | Global rules path                              | Shape     |
| -------------- | ------------------------------------ | ---------------------------------------------- | --------- |
...
| OpenClaw       | — (no project rules)                 | `~/.openclaw/workspace/AGENTS.md`              | aggregate |
...
[rule source reference](/docs/configuration#rule-source-fields) for how rules are transformed and merged.
