...
```
1. Load config (`$KASETTO_CONFIG` → `./kasetto.yaml` → `source:` in `$XDG_CONFIG_HOME/kasetto/config.yaml` → `$XDG_CONFIG_HOME/kasetto/kasetto.yaml`, or explicit `--config` path/URL)
...
Skills are fully replaced on update. There are no partial merges.
...
| ZCode JSON           | ZCode (nested `mcp.servers`)                     | `.zcode/cli/config.json`           |
...
`kasetto.lock` is a real lockfile, in the same spirit as `Cargo.lock`, `package-lock.json`, or `go.sum`. It pins exactly which skills, commands, instructions, and MCP assets were installed (by source, revision, destination, and content hash), so anyone who runs `kst sync` against the same config and lock gets an identical setup.
...
- It contains **no timestamps or machine-specific data**, so nothing run-specific leaks in.
...
| Command                            | Behavior                                                                                                                              |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `kst sync`                         | Installs exactly what the lock pins. If the on-disk destinations already match the locked hashes, **no network fetch happens**, and moving refs like a branch are not re-resolved. |
...
| `kst sync --update-local`          | Re-resolves every **local-path** source (a `source:` that is a directory, not a URL) and rewrites its pins, no names needed. Remote sources are untouched. |
...
A plain `kst sync` always re-hashes the destination contents locally (cheap, no network) and repairs any tampered or missing copy from the lock, but it will not phone home to check whether an upstream branch moved. That is what `--update` is for. `--update` and `--locked` together are contradictory and rejected; so are `--update-local` and `--locked`.

Local directory sources are pinned by the lock exactly like remote ones: edit a skill under `./skills/` and a plain sync still serves the locked copy. `--update-local` is the escape hatch scoped to just those sources. It re-reads every local directory and rewrites its pins while leaving remote branch pins alone, so shipping your own edits never drags in upstream churn. Combine it with `--update <name>` to refresh a named remote in the same run.
