...
`kasetto.lock` is a real lockfile, in the same spirit as `Cargo.lock`, `package-lock.json`, or `go.sum`. It pins exactly which skills, commands, instructions, and MCP assets were installed — by source, revision, destination, and content hash — so that anyone who runs `kst sync` against the same config and lock gets an identical setup.
