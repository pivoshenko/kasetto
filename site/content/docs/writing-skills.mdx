...
`SKILL.md` is a markdown file. YAML frontmatter is optional but gives you control over the metadata Kasetto records for the skill:
...
| ------------- | -------- | -------------------------------------------------- |
| `name`        | no       | Display name carried in `kst list --json` output   |
