...
Runs `gcloud secrets versions access latest --secret=<name>` (the [gcloud CLI](https://cloud.google.com/sdk/gcloud)) against the active project. Example: `${kst:gcp:db-password}`.
...
Runs `az keyvault secret show --vault-name <vault> --name <name>` (the [Azure CLI](https://learn.microsoft.com/cli/azure/)). Example: `${kst:az:my-vault/db-password}`.
