...
Kasetto tracks everything it installed in a YAML lock file called `kasetto.lock`. Where it lives depends on the scope:
...
The lock file is how Kasetto knows what to remove when you drop a source from the config or run `kst clean`. You generally won't need to touch it by hand.
