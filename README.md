...
- **`kst sync`** — read config, install skills + MCPs + commands + instructions into agent dirs honoring `kasetto.lock`; `--update` rolls pins forward, `--locked`/`--frozen` enforce the lock without fetching, `--audit`/`--no-audit` toggle the skills.sh security gate.
...
- **`kst audit`** — check installed skills against [skills.sh](https://skills.sh) security verdicts; reports the worst partner verdict per skill, `--verbose` shows the per-partner breakdown. Advisory by default; opt into enforcement with `audit.fail-on` in config or `sync --audit`.
