...
- **`kst sync`**: read config, install skills + MCPs + commands + instructions into agent dirs honoring `kasetto.lock`; `--update` rolls pins forward, `--update-local` re-resolves only local directory sources (ship your own edits without touching remote pins), `--locked`/`--frozen` enforce the lock without fetching
