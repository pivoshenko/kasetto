...
Kasetto is a single-binary CLI tool that syncs AI agent assets — skills, slash-commands, MCP servers, and rules (`CLAUDE.md` / `.cursor/rules` / `AGENTS.md` …) — from GitHub repos or local directories into 21 agent environments. Two binaries (`kasetto` and `kst`) share the same code.
...
- **`commands/`** - Each subcommand: `sync/` (split into `skills.rs` + `mcps.rs` + `commands.rs` + `rules.rs`), `add`, `remove`, `lock`, `list`, `doctor`, `init`, `clean`, `self_update`, `uninstall`, `completions`. `add`/`remove` rewrite the local config via `fsops::config_edit` (comment-preserving surgical YAML edits) then delegate to `sync` (shared plumbing in `source_edit.rs`); `lock` re-resolves + pins `kasetto.lock` without installing (skill source-tree hash equals install hash, so `sync --locked` works offline afterwards; MCP/command entries get revision pins only)
- **`model/`** - Core types: `Agent` enum (21 presets with install paths), `Config` (YAML deserialization), `Scope` (Global/Project), `SkillEntry`, `CommandEntry`, `CommandFormat`, `RuleEntry`, `RulesField`, `RuleSourceSpec`, `RuleFormat` (`AggregateMarkdown`/`CursorMdc`/`PlainMarkdownDir`), `RuleTarget`, `Report`, `Summary`. `extend.rs` holds the YAML-level `extends` merge: scalars replace; `skills`/`mcps`/`commands`/`rules` merge by `(source, ref|branch, sub-dir)` identity
...
- **`frontmatter.rs`** - Shared Markdown-with-YAML-frontmatter parser (`Parsed` + `parse`), consumed by both `prompts` and `rules` (extracted from the old `prompts/parse.rs`)
...
- **`rules/`** - Rule (agent instruction file) handling: per-agent transforms (`transform.rs`), entry point `apply_rule`; parses via `crate::frontmatter`. Two destination shapes: **aggregate** (`AggregateMarkdown` — many rules merge into one shared `CLAUDE.md`/`AGENTS.md`/`GEMINI.md`/`.github/copilot-instructions.md` via managed `<!-- kasetto:rule:ID … -->` comment blocks so user hand-edits and other rules survive) and **per-rule directory** (`CursorMdc` reconstructs `description`/`globs`/`alwaysApply` MDC frontmatter; `PlainMarkdownDir` emits body only for `.windsurf/rules`, `.clinerules`, `.continue/rules`, etc.). The block id is `hash(source::name)`-keyed so same-named rules from different sources don't collide; the lock `destination` token is `agg:<rel>` (teardown strips the block, never deletes the file) or `file:<rel>` (teardown deletes the file). Per-agent destinations live in `model::agent::rules_project_path`/`rules_global_path`, verified against each agent's official docs (e.g. Claude `CLAUDE.md`, Codex/OpenCode/Amp `AGENTS.md`, Cursor `.cursor/rules/*.mdc`, Copilot `.github/copilot-instructions.md`, Gemini/Antigravity `GEMINI.md`, Replit `replit.md`, OpenHands `.openhands/microagents/repo.md`, Junie `.junie/AGENTS.md`; OpenClaw has no project rules; Warp/Trae have UI-managed globals → `None`; Cursor global `~/.cursor/rules` is community-supported `.mdc`). Targeting follows the config-level `agent:` list, identical to skills/commands — no per-rule agent filter
- **`lock.rs`** - Portable manifest persistence (`kasetto.lock`, schema `version: 3` — bumped from 2 when the `rules` asset kind joined): tracks installed skills + command/MCP/rule assets. Rule assets use `kind: "rules"`; the `destination` CSV stores `agg:`/`file:`-prefixed tokens (see the `rules/` module). Deterministic and commit-friendly — `destination` paths are stored relative to the scope root (project root for Project, home for Global) and resolved back to absolute at read time via `fsops::resolve_dest`/`relativize_dest`. No timestamps or run-specific data. **The lock is authoritative** (issue #33): a plain `kasetto sync` installs exactly what the lock pins and performs zero network fetches when on-disk destinations already match. The per-source `needs_fetch` gate in `commands/sync/skills.rs` re-hashes every destination *before* downloading and skips `materialize_source` entirely when all destinations match the locked hash; a missing/tampered secondary destination is repaired by copying from a verified-good local destination rather than fetching. `--update`/`-u` (optionally `--update <name>...`) is the only path that re-resolves moving refs and rewrites locked hashes; for a `skills: "*"` wildcard source, plain sync holds to the locked set (derived from lock entries where `entry.source == src.source`) and only `--update` re-resolves the wildcard via download + `select_targets`. `--locked`/`--frozen` never fetches and errors when the lock cannot satisfy the config (config-named skill absent, or source entirely absent); `--locked --update` is rejected as contradictory. Skill content hashing (`fsops::hash_dir`) normalizes path separators to `/` so digests are OS-invariant — existing locks therefore show one round of `updated` on the next plain sync after upgrading.
...
5. For each rule source: materialize → discover rules (`rules: "*"` auto-discovers `rules/**/*.{md,mdc}`; named/explicit entries otherwise) → parse frontmatter → transform per agent (`rules::apply_rule`) → write (managed-block upsert for aggregate files, per-rule file for dir formats) → update lock
6. For each MCP source: materialize → resolve files (`mcps: "*"` → auto-discover `.mcp.json` / `mcp.json` / `mcps/*.json`; `mcps: [names]` → `mcps/<name>.json`; `mcps: [{name, path}]` → `<path>/<name>.json`) → collect pending installs → merge into agent settings files → update lock
